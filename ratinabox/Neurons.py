...
class FieldOfViewOVCells(ObjectVectorCells):
    ...
    def __init__(self,Agent,params={}):
        ...
        # TODO: have some assertions on the manifold to make sure it is valid
        ...
        for i, obj_id in enumerate(unique_objects):
            ...
            self.tuning_types[sid:eid] = obj_id * np.ones(manifold["n"])
            self.tuning_distances[sid:eid] = manifold["mu_d"]
            self.sigma_distances[sid:eid] = manifold["sigma_d"]
            self.tuning_angles[sid:eid] = manifold["mu_theta"]
            self.sigma_angles[sid:eid] = manifold["sigma_theta"]
    ...
    def get_manifold(
        ...
        """Returns the manifold of parameters for the B/OVCs tiling the Agents FoV.
         ...
         Args:
            ...
            • manifold_function (str): "uniform" (all receptive fields the same size or "hartley" 
