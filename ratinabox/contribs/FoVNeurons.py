...
class FoVNeurons(Neurons):
    """FOV_BVCs are collection of boundary vector cells organised so as to represent the local field of view i.e. what walls the agent can "see" in the local vicinity. They work by using as follow:

    A "manifold" of boundary vector cells (BVCs) tiling the agents FoV is initialised. Users define the radius and angular extent of this manifold which determine the distances and angles available in htte FoV. These default to a 240 degree FoV from distance of 0 to 20cm. Each point on this manifold is therefore defined  by tuple (angle and radius), or (θ,r). Egocentric BVCs are initialised which tile this manifold uniformly, the BVC at position (θ,r) will fire with a preference for boundaries a distance r from the agent at an angle θ relative to the current heading direction (they are "egocentric"). The position on the manifold determines the angular and distance prefences of the BVC in egocentric coordinates. Thus only if the part of the manifold where the cell sits crosses a wall, will that cell fire. Thus only the cells on the part of a manifold touching or close to a wall will fire.
    ...
    def __init__(self, Agent, params={}):
        default_params = {
            ...
            "spatial_resolution": 0.05,  # resolution of each BVC tiling FoV
